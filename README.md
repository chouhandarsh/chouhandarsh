<div align="center">

<!-- Dynamic Typing Header -->
[![Typing SVG](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=28&duration=3000&pause=1000&color=00D4FF&center=true&vCenter=true&multiline=true&repeat=false&width=700&height=80&lines=Darsh+Chouhan)](https://git.io/typing-svg)

[![Typing SVG](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=400&size=18&duration=2500&pause=1000&color=A0AEC0&center=true&vCenter=true&width=700&height=50&lines=AI+Developer+%7C+ML+Engineer+in+Training;Building+Intelligence%2C+One+Model+at+a+Time;Learn+the+math.+Build+from+scratch.+Ship+it.)](https://git.io/typing-svg)

<br/>

<!-- Futuristic AI-themed banner -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0D1117,50:00D4FF,100:7928CA&height=200&section=header&text=AI%20%2F%20ML%20Engineer&fontSize=52&fontColor=FFFFFF&fontAlign=50&fontAlignY=38&desc=IIIT%20Dharwad%20%7C%20India&descSize=18&descAlign=50&descAlignY=60&animation=twinkling"/>

</div>

---

## `whoami`

```python
class DarshChouhan:
    role        = "AI Developer & CS Undergraduate"
    college     = "IIIT Dharwad, India"
    philosophy  = "Learn the math → Understand the algo → Build from scratch → Use the framework"

    focus = [
        "Large Language Models & RAG Pipelines",
        "Machine Learning from First Principles",
        "Deep Learning & Neural Architectures",
        "Production-grade AI Systems",
    ]

    currently_building = [
        "RAG-powered document intelligence systems",
        "LLM applications with LangChain",
        "ML models trained from mathematical foundations",
    ]

    goal = "Ship AI systems that are robust, interpretable, and impactful."
```

> *I don't chase frameworks. I chase understanding.*

---

## ⚙️ Tech Stack

<div align="center">

**Languages**

![Python](https://img.shields.io/badge/Python-0D1117?style=for-the-badge&logo=python&logoColor=00D4FF)
![C++](https://img.shields.io/badge/C++-0D1117?style=for-the-badge&logo=cplusplus&logoColor=00D4FF)

**ML & Data**

![NumPy](https://img.shields.io/badge/NumPy-0D1117?style=for-the-badge&logo=numpy&logoColor=00D4FF)
![Pandas](https://img.shields.io/badge/Pandas-0D1117?style=for-the-badge&logo=pandas&logoColor=00D4FF)
![Matplotlib](https://img.shields.io/badge/Matplotlib-0D1117?style=for-the-badge&logo=plotly&logoColor=00D4FF)
![Scikit-Learn](https://img.shields.io/badge/Scikit--Learn-0D1117?style=for-the-badge&logo=scikitlearn&logoColor=00D4FF)

**AI Engineering**

![LangChain](https://img.shields.io/badge/LangChain-0D1117?style=for-the-badge&logo=chainlink&logoColor=7928CA)
![ChromaDB](https://img.shields.io/badge/ChromaDB-0D1117?style=for-the-badge&logo=databricks&logoColor=7928CA)
![Vector DBs](https://img.shields.io/badge/Vector_Databases-0D1117?style=for-the-badge&logo=pinecone&logoColor=7928CA)
![RAG](https://img.shields.io/badge/RAG_Pipelines-0D1117?style=for-the-badge&logo=openai&logoColor=7928CA)

**Tools**

![Git](https://img.shields.io/badge/Git-0D1117?style=for-the-badge&logo=git&logoColor=F05032)
![Linux](https://img.shields.io/badge/Linux-0D1117?style=for-the-badge&logo=linux&logoColor=FCC624)
![VS Code](https://img.shields.io/badge/VS_Code-0D1117?style=for-the-badge&logo=visualstudiocode&logoColor=007ACC)
![GitHub](https://img.shields.io/badge/GitHub-0D1117?style=for-the-badge&logo=github&logoColor=ffffff)

</div>

---

## 🧭 Learning Roadmap

<details open>
<summary><b>Machine Learning — In Progress</b></summary>
<br/>

- [x] Linear & Multiple Linear Regression
- [x] Cost Functions & Loss Landscapes
- [x] Gradient Descent & Variants
- [x] Model Evaluation & Metrics
- [x] Data Preprocessing & Feature Engineering
- [ ] Regularization (L1, L2, ElasticNet)
- [ ] Advanced Optimization Algorithms

</details>

<details open>
<summary><b>AI Engineering — In Progress</b></summary>
<br/>

- [x] LLM Architecture & Prompt Engineering
- [x] Retrieval-Augmented Generation (RAG)
- [x] Embeddings & Semantic Search
- [x] Vector Databases (ChromaDB)
- [x] LangChain Pipelines
- [ ] Fine-tuning & RLHF
- [ ] Agents & Tool Use

</details>

<details>
<summary><b>Deep Learning — Starting</b></summary>
<br/>

- [ ] Neural Networks from scratch (NumPy)
- [ ] Backpropagation & Automatic Differentiation
- [ ] CNNs & Computer Vision
- [ ] Transformers & Attention Mechanisms
- [ ] Generative Models (VAEs, Diffusion)

</details>

<details>
<summary><b>DSA — Ongoing</b></summary>
<br/>

- [x] Dynamic Programming
- [x] Graph Algorithms (BFS, DFS, Dijkstra)
- [x] Trees & Binary Search
- [x] Recursion & Backtracking
- [x] Greedy Algorithms
- [ ] Advanced Graph Theory
- [ ] Segment Trees & Advanced DS

</details>

---

## 🛠️ Featured Projects

> Projects that demonstrate building AI from first principles — not just importing libraries.

<div align="center">

| Project | Stack | Description |
|:--------|:------|:------------|
| 🔍 **RAG Document Intelligence** | LangChain · ChromaDB · Python | PDF Q&A system with semantic retrieval and LLM-powered answers |
| 🤖 **ML From Scratch** | Python · NumPy | Linear regression, gradient descent, and cost functions — pure math, no sklearn |
| 💬 **LLM Knowledge Assistant** | LangChain · Vector DB | Domain-specific assistant with document embedding and context retrieval |
| 📊 **Data Analysis Pipeline** | Pandas · Matplotlib · Scikit-Learn | End-to-end pipeline: preprocessing → modeling → evaluation |

</div>

*More projects dropping regularly. Watch this space.*

---

## 📊 GitHub Stats

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=DarshChouhan&show_icons=true&theme=github_dark&bg_color=0D1117&border_color=00D4FF&title_color=00D4FF&icon_color=7928CA&text_color=A0AEC0&hide_border=false&include_all_commits=true&count_private=true"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=DarshChouhan&layout=compact&theme=github_dark&bg_color=0D1117&border_color=00D4FF&title_color=00D4FF&text_color=A0AEC0&hide_border=false&langs_count=6"/>

</div>

<div align="center">

[![GitHub Streak](https://streak-stats.demolab.com?user=DarshChouhan&theme=dark&background=0D1117&border=00D4FF&ring=7928CA&fire=00D4FF&currStreakLabel=00D4FF&sideLabels=A0AEC0&dates=A0AEC0&currStreakNum=FFFFFF&sideNums=FFFFFF)](https://git.io/streak-stats)

</div>

---

## 🏆 Trophies

<div align="center">

[![trophy](https://github-profile-trophy.vercel.app/?username=DarshChouhan&theme=darkhub&no-frame=true&margin-w=10&column=6&title=Commits,Repositories,Stars,Followers,Issues,PullRequest)](https://github.com/ryo-ma/github-profile-trophy)

</div>

---

## 📈 Contribution Graph

<div align="center">

[![Darsh's Contribution Graph](https://github-readme-activity-graph.vercel.app/graph?username=DarshChouhan&bg_color=0D1117&color=00D4FF&line=7928CA&point=FFFFFF&area=true&hide_border=true)](https://github.com/ashutosh00710/github-readme-activity-graph)

</div>

---

## 🐍 Contribution Snake

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/DarshChouhan/DarshChouhan/output/github-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/DarshChouhan/DarshChouhan/output/github-snake.svg" />
  <img alt="github contribution grid snake animation" src="https://raw.githubusercontent.com/DarshChouhan/DarshChouhan/output/github-snake.svg" />
</picture>

</div>

---

## 💡 Engineering Principle

<div align="center">

> *"First principles beat frameworks. Every time."*
>
> Understand the gradient before you call `.fit()`.
> Know the attention mechanism before you import a transformer.
> Build the retriever before you wire up the chatbot.

</div>

---

## 🤝 Connect

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0D1117?style=for-the-badge&logo=linkedin&logoColor=0077B5)](https://linkedin.com/in/darshchouhan)
[![LeetCode](https://img.shields.io/badge/LeetCode-0D1117?style=for-the-badge&logo=leetcode&logoColor=FFA116)](https://leetcode.com/darshchouhan)
[![GeeksForGeeks](https://img.shields.io/badge/GeeksForGeeks-0D1117?style=for-the-badge&logo=geeksforgeeks&logoColor=2F8D46)](https://auth.geeksforgeeks.org/user/darshchouhan)
[![Email](https://img.shields.io/badge/Email-0D1117?style=for-the-badge&logo=gmail&logoColor=EA4335)](mailto:darshchouhan@gmail.com)

</div>

---

<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:7928CA,50:00D4FF,100:0D1117&height=120&section=footer"/>

</div>
